...
namespace DesysiaLOD {
	[CustomEditor(typeof(DesysiaLODEditor))]
	class InnerDesysiaLODEditor: Editor {
		// run when focused
						...
						if (GUI.changed) {
							if(save_show_help != show_help) {
								...
								if(show_help) {
									...
									if(!System.IO.File.Exists(filename)) System.IO.File.Create(filename).Dispose(); ;
								}
			...
			if (Desysia_Meshes == null) {
				if(target) {
					...
					if (Desysia_Meshes != null) {
						...
						string filename2 = "desysialod_not_show_help";
